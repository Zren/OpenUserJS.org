  ...
  async.waterfall([
    ...
    function (aBlobUtf8, aCallback) {
      ...
      var onScriptStored = function (aScript) {
        if (aScript) {
          // Update the Script document with the current GitHub filepath.
          // TODO: Weave this logic into scriptStorage.storeScript() so we only write once.
          aScript.githubSyncUserId = githubUserId;
          aScript.githubSyncRepoName = githubRepoName;
          aScript.githubSyncSourcePath = githubBlobPath;

          aScript.save(function(aErr, aNumberAffected, aRawResponse) {
            data.script = aScript;
            aCallback(aErr, data);
          });
        } else {
...
var parseMarkdownBlob = function (aMarkdownBlob) {
  ...
  if (aMarkdownBlob.size > settings.maximum_script_description_size) {
    aMarkdownBlob.errors.push({
      msg: util.format('File size is larger than maximum (%s bytes).', settings.maximum_script_description_size)
    });
  }
...
var importMarkdownBlob = function (aOptions, aCallback) {
  ...
  async.waterfall([
    ...
    function (aBlobUtf8, aCallback) {
      // Double check file size.
      if (aBlobUtf8.length > settings.maximum_script_description_size)
        return aCallback(util.format('File size is larger than maximum (%s bytes).', settings.maximum_script_description_size));

      // script.about =
      Script.update({
        githubSyncDescription: true,
        githubSyncUserId: githubUserId,
        githubSyncRepoName: githubRepoName,
        githubSyncDescriptionPath: githubBlobPath
      }, {
        about: aBlobUtf8
      }, {
        multi: true
